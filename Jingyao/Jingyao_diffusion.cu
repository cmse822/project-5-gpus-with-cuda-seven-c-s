...
void cuda_diffusion(float* u, float *u_new, const unsigned int n){
  ...
  if (i < n - NG) {
    u_new[i] = u[i] + c_a * (-u[i-2] + 8.f*u[i-1] - 8.f*u[i+1] + u[i+2]);
  }
  ...
  if (threadIdx.x == 0) {
    ...
    if (blockIdx.x == gridDim.x - 1) { // Right boundary
      u_new[n - NG] = -u_new[n - NG - 1];
      u_new[n - NG + 1] = -u_new[n - NG - 2];
    }
  }
...
void shared_diffusion(float* u, float *u_new, const unsigned int n){
  ...
  int globalIdx = blockIdx.x * blockDim.x + threadIdx.x + NG;
  // Calculate the local index in the shared memory
  int localIdx = threadIdx.x + NG; // Assuming halo of NG elements
  ...
  if (threadIdx.x < NG) {
    ...
    // Load right halo
    shared_u[localIdx + blockDim.x] = u[globalIdx + blockDim.x];
  }

  // Synchronize the threads in the block to make sure shared memory is populated
  __syncthreads();

  // Do the diffusion calculation using the shared memory
  if (globalIdx < n - NG) { // Ensure we are not out of bounds
    float sum = -c_a * shared_u[localIdx - 2] + 
                 c_b * shared_u[localIdx - 1] - 
                 c_c * shared_u[localIdx] +
                 c_b * shared_u[localIdx + 1] -
                 c_a * shared_u[localIdx + 2];
    u_new[globalIdx] = shared_u[localIdx] + sum;
  }

  // Synchronize again before modifying global memory
  __syncthreads();
